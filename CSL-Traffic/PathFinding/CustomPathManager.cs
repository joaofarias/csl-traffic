...
namespace CSL_Traffic
{
	...
	public class CustomPathManager : PathManager
	{
        ...
        public static bool FindPathPosition(Vector3 position, ItemClass.Service service, NetInfo.LaneType laneType, VehicleInfo.VehicleType vehicleTypes, float maxDistance, out PathUnit.Position pathPos, RoadManager.VehicleType vehicleType)
        {
            PathUnit.Position position2;
            float num;
            float num2;
            return CustomPathManager.FindPathPosition(position, service, laneType, vehicleTypes, maxDistance, out pathPos, out position2, out num, out num2, vehicleType);
        }
        public static bool FindPathPosition(Vector3 position, ItemClass.Service service, NetInfo.LaneType laneType, VehicleInfo.VehicleType vehicleTypes, float maxDistance, out PathUnit.Position pathPosA, out PathUnit.Position pathPosB, out float distanceSqrA, out float distanceSqrB, RoadManager.VehicleType vehicleType)
        {
            ...
            for (int i = num2; i <= num4; i++)
            {
                for (int j = num; j <= num3; j++)
                {
                    ...
                    while (num6 != 0)
                    {
                        ...
                        if (info.m_class.m_service == service && (instance.m_segments.m_buffer[(int)num6].m_flags & NetSegment.Flags.Flooded) == NetSegment.Flags.None)
                        {
