...
namespace CSL_Traffic
{
	...
	class CustomPathFind : PathFind
	{
		...
		{
			get { return (CustomPathFind.BufferItem[])fi_buffer.GetValue(this); }
			set { fi_buffer.SetValue(this, value); }
		}
		...
		private bool m_isServiceVehicle;		
		...
		private void PathFindImplementation(uint unit, ref PathUnit data)
		{
			...
			this.m_isServiceVehicle = ((this.m_pathUnits.m_buffer[(int)((UIntPtr)unit)].m_simulationFlags & 8) != 0);
		...
		private void ProcessItem(CustomPathFind.BufferItem item, ushort targetNode, bool targetDisabled, ushort segmentID, ref NetSegment segment, uint lane, byte offset, byte connectOffset)
		{
			...
			while (num8 < num && num2 != 0u)
			{
				if (lane == num2)
				{
					...
					if (CheckLaneType(lane3, this.m_laneTypes, this.m_vehicleTypes))
					{
		...
		private bool ProcessItem(CustomPathFind.BufferItem item, ushort targetNode, ushort segmentID, ref NetSegment segment, ref int currentTargetIndex, byte connectOffset, bool enableVehicle, bool enablePedestrian)
		{
			...
			while (num12 < num && num2 != 0u)
			{
				...
				{
					if (CheckLaneType(lane2, laneType2, this.m_vehicleTypes) && (segmentID != item.m_position.m_segment || num12 != (int)item.m_position.m_lane) && (byte)(lane2.m_finalDirection & direction2) != 0)
					{
						Vector3 a;
						if ((byte)(direction & NetInfo.Direction.Forward) != 0)
						{
		...
		private void GetLaneDirection(PathUnit.Position pathPos, out NetInfo.Direction direction, out NetInfo.LaneType type)
		{
			...
			if (info.m_lanes.Length > (int)pathPos.m_lane)
			{
				...
				//if (m_isServiceVehicle && (type & NetInfo.LaneType.Pedestrian) != 0)
				//{
				//	type &= ~NetInfo.LaneType.Pedestrian;
				//	type |= NetInfo.LaneType.Vehicle;
				//}
		...
		public bool CheckLaneType(NetInfo.Lane lane, NetInfo.LaneType laneTypes, VehicleInfo.VehicleType vehicleTypes)
		{
			if (laneTypes == NetInfo.LaneType.None || (byte)(laneTypes & lane.m_laneType) != 0)
			{
				...
				if (vehicleTypes == VehicleInfo.VehicleType.None || (vehicleTypes & lane.m_vehicleType) != VehicleInfo.VehicleType.None)
				{
					return true;
				}
			}
